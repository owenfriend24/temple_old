...
def sort_files_by_acquisition(data_dir, subj_id):
    ...
    for file in files:
        ...
        df = pd.DataFrame([fn, acq]).T
        df.columns = ['Filename', 'AcquisitionTime']
...
def get_fieldmap_mapping(data_dir, subj_id):
    ...
    for ind, row in df.T.iteritems():
        run_id = row.Filename.split('_')[-1]

        # identify fieldmap files
        if (run_id[:3] == 'mag') or (run_id[:3] == 'pha'):
            new_fieldmap_run_num = int(row.Filename.split('_')[1].split('-')[-1])

            # if new fieldmap run number, save out runs and create new empty run list
    ...
    # save out final fieldmap run list
