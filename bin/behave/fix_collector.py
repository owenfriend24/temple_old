...
def main(data_dir, sub):
    ...
    ref = pd.read_table(func_dir + f'sub-temple{sub}_task-arrow_run-01_events.tsv')

    c1 = pd.read_table(func_dir + f'sub-temple{sub}_task-collector_run-01_events.tsv')
    c2 = pd.read_table(func_dir + f'sub-temple{sub}_task-collector_run-02_events.tsv')
    c3 = pd.read_table(func_dir + f'sub-temple{sub}_task-collector_run-03_events.tsv')
    c4 = pd.read_table(func_dir + f'sub-temple{sub}_task-collector_run-04_events.tsv')
    ...
    for col_run in [c1, c2, c3, c4]:
        for index, row in col_run.iterrows():
            ...
            out = (func_dir + f'sub-temple{sub}_task-collector_run-0{run}_events_fixed.tsv')
